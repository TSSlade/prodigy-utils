...
/* http://wavesurfer-js.org/example/stretcher/soundtouch.js
 ...
 */
console.log("Loading soundtouch.js");
(function(window) {
    /**
...
console.log("Loading stretcher.js");
// http://wavesurfer-js.org/example/stretcher/app.js
'use strict';

// Create an instance
// var wavesurfer_prod = {};
console.log('Pausing to allow DOM loading to complete');
setTimeout(function() {
// Init & load
// window.addEventListener('DOMContentLoaded', function() {
    /*
    ** Removed because a wavesurfer object has already been
    ** instantiated by Prodigy
    ...
    */
    ...
    timestretcher = window.wavesurfer;
    // Time stretcher
    /*
    ** If this wavesurfer.on('ready') is enabled the script won't work

    window.wavesurfer.on('ready', function() {
    
    */
        var st = new window.soundtouch.SoundTouch(
            timestretcher.backend.ac.sampleRate
        );

        var buffer = timestretcher.backend.buffer;
        ...
        timestretcher.on('play', function() {
            // console.log('Within the on.play function...')
            seekingPos = ~~(timestretcher.backend.getPlayedPercents() * length);
            st.tempo = timestretcher.getPlaybackRate();

            if (st.tempo === 1) {
                timestretcher.backend.disconnectFilters();
            } else {
                if (!soundtouchNode) {
                    var filter = new window.soundtouch.SimpleFilter(source, st);
                    soundtouchNode = window.soundtouch.getWebAudioNode(
                        timestretcher.backend.ac,
                        filter
                    );
                }
                timestretcher.backend.setFilter(soundtouchNode);
            }
        });

        timestretcher.on('pause', function() {
            // console.log('Within the on.pause function...')
            soundtouchNode && soundtouchNode.disconnect();
        });

        timestretcher.on('seek', function() {
            // console.log('Within the on.seek function...')
            seekingPos = ~~(timestretcher.backend.getPlayedPercents() * length);
        });
    // });  // Matches the window.wavesurfer.on('ready')
// });  // Matches the window.addEventListener
}, 1000);